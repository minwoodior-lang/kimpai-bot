  ...
  return (
    <Layout>
      <Head>
        ...
        <meta name="description" content={`${coinInfo.name} (${symbolStr}) 실시간 김치프리미엄 분석, 업비트/바이낸스 가격 비교`} />
      </Head>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-4">
            <div className="text-slate-400 text-sm mb-1">업비트 가격</div>
            <div className="text-xl font-bold text-white">₩{coinData.upbitPrice.toLocaleString()}</div>
          </div>
          <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-4">
            <div className="text-slate-400 text-sm mb-1">바이낸스 (원화 환산)</div>
        ...
        <div className="grid lg:grid-cols-3 gap-6 mb-8">
          <div className="lg:col-span-2">
            <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl overflow-hidden">
              <div className="flex items-center justify-between p-4 border-b border-slate-700/50">
                <h3 className="text-white font-medium">Binance {symbolStr}/USDT</h3>
                <span className="text-slate-400 text-sm">1시간 차트</span>
              </div>
              <TradingViewChart symbol={symbolStr} exchange="BINANCE" height={400} />
            </div>
          </div>
          <div className="space-y-4">
            ...
            <PremiumHistoryChart symbol={symbolStr} hours={24} />
          </div>
        </div>

        <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl overflow-hidden">
          <div className="p-4 border-b border-slate-700/50">
            <h3 className="text-white font-medium">Upbit {symbolStr}/KRW</h3>
          </div>
          <TradingViewChart symbol={symbolStr} exchange="UPBIT" height={300} />
        </div>
      </div>
    </Layout>
  );
