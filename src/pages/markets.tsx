...
export default function Markets() {
  ...
  useEffect(() => {
    const loadAllMarkets = async () => {
      try {
        const res = await fetch("/api/master-symbols");
        if (res.ok) {
    ...
    <Layout>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mt-12 pt-8 border-t border-slate-700/50">
          <h2 className="text-2xl font-bold text-white mb-6">모든 거래 심볼</h2>
            ...
            <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-slate-700/50">
                      <th className="text-left text-slate-400 font-medium px-6 py-4">심볼</th>
                      <th className="text-left text-slate-400 font-medium px-6 py-4">한글명</th>
                      <th className="text-left text-slate-400 font-medium px-6 py-4">영문명</th>
                    </tr>
                  </thead>
                  <tbody>
                    {allMarkets.map((market) => (
                      <tr
                        ...
                        onClick={() => router.push(`/markets/${market.symbol.toLowerCase()}`)}
                      >
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            {market.icon_path ? (
                              <Image
                                src={market.icon_path}
                                alt={market.symbol}
                                width={32}
                                height={32}
